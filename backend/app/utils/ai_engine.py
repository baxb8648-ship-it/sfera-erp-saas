...
def generate_rag_answer(query: str, context_chunks: List[dict], model_name: Optional[str] = None) -> str:
    ...
    if not context_chunks:
        prompt = f"""Ты — интеллектуальный бизнес-ассистент СФЕРУМ.
...
Ответ на русском языке:"""
        ...
        prompt = f"""Ты — интеллектуальный бизнес-ассистент СФЕРУМ. Ответь на вопрос пользователя на русском языке, опираясь СТРОГО на предоставленные ниже фрагменты из корпоративной базы знаний.
Правила:
...
def ai_classify_intent(text: str) -> str:
    ...
    """
    prompt = f"""Ты — ИИ-аналитик CRM-системы СФЕРУМ. Твоя задача — классифицировать намерение пользователя по его тексту.
Доступные намерения:
