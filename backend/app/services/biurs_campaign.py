...
def generate_biurs_commercial_offer(customer_name: str, tender_title: str, available_qty: float = 0.0) -> str:
    ...
    prompt = f"""Составь высокопрофессиональное коммерческое предложение от компании ООО «СФЕРА» для заказчика {customer_name}.
...
Письмо:"""
    ...
    if not response:
        ...
        response = (
            ...
            f"ООО «СФЕРА» предлагает к поставке двухкомпонентное антикоррозийное покрытие БИУРС "
